...
static inline int ar0820_read_reg(struct camera_common_data *s_data, u16 addr,
				  u16 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
static inline int ar0820_write_reg(struct camera_common_data *s_data, u16 addr,
				   u16 val)
{
	int err = 0;
...
static int ar0820_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	/* fixed fps per mode */
	priv->frame_length = AR0820_DEFAULT_FRAME_LENGTH;
	return 0;
}
...
ar0820_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct device_node *np = dev->of_node;
	struct camera_common_pdata *board_priv_pdata;
	struct ar0820 *ar0820_s;
...
static int ar0820_board_setup(struct ar0820 *priv)
{
	...
	if (dser_i2c->dev.driver == NULL) {
		dev_err(dev,
			"missing deserializer driver - ask for reprobe?\n");
		//return -EPROBE_DEFER;
		goto error;
	}
...
static const struct i2c_device_id ar0820_id[] = { { "ar0820", 0 }, {} };
